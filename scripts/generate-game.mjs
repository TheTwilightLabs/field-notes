...
const outDir = path.join(root, "public", "game", "signal-garden");
fs.mkdirSync(outDir, { recursive: true });

const mechanics = [
  ["Player state", "Create player variables and print a status panel."],
  ["World map", "Represent connected locations with dictionaries and collections."],
  ["Command rules", "Validate movement and branch on commands."],
  ["Game loop", "Keep processing commands until a win, loss, or quit."],
  ["Action functions", "Separate movement, listening, resting, and status."],
  ["Player object and saves", "Package state in a class and save progress as JSON."],
  ["Complete game", "Combine every system and add your own extension."],
];

function source(chapter, starter) {
  const [mechanic, task] = mechanics[chapter - 1];
  return `#!/usr/bin/env python3
"""Signal Garden — Chapter ${chapter}: ${mechanic}

Run interactively:
    python chapter-${String(chapter).padStart(2, "0")}-${starter ? "starter" : "completed"}.py

Run the deterministic verification demo:
    python chapter-${String(chapter).padStart(2, "0")}-${starter ? "starter" : "completed"}.py --demo
"""
...
def run_demo():
    ...
    print(f"SIGNAL GARDEN / CHAPTER {CHAPTER} / ${mechanic.toUpperCase()}")
...
def chapter_task():
    """${task}"""
${starter ? "    # TODO: Complete this chapter's mechanic, then compare with the completed file.\n    pass" : `    return "${mechanic} complete"`}
...
for (let chapter = 1; chapter <= mechanics.length; chapter += 1) {
  const number = String(chapter).padStart(2, "0");
  fs.writeFileSync(path.join(outDir, `chapter-${number}-starter.py`), source(chapter, true));
  fs.writeFileSync(path.join(outDir, `chapter-${number}-completed.py`), source(chapter, false));
}

console.log(`Generated ${mechanics.length * 2} Signal Garden files in ${path.relative(root, outDir)}`);
